 ...
 *
 * ★ 카운트가 아니라 **거리로** 재야 한다. 이걸 처음에 놓쳤다.
 *
 *   곡선 환산이 들어가면서 같은 카운트 편차가 행정 맨 위에서 크게 부풀려진다 —
 *   바닥의 여섯 배쯤이다. "p-p 40 카운트니까 0.06mm" 라고 잡았던 것이 실제로는
 *   0.10mm 였고, 문턱이 잡음 바닥에 딱 걸쳐서 켜졌다 꺼졌다 했다.
 *
 *   장치에서 60초(175만 회 스캔) 재서 얻은 값 (`keys noise 60000`) —
 *
 *     대부분 셀      0.02 ~ 0.06 mm
 *     최악 한 셀     0.10 mm
 *
 *   그 위에서 자른다. 값을 두 군데 적으면 한쪽만 고치게 되므로 여기 한 곳에 둔다.
 */
const HE_MM_MIN = 14;
