...
위쪽 키보드 그림은 창 중앙에 그려지는데 아래 설정 칸은 왼쪽 메뉴 폭만큼 밀려 있었다.
중심이 어긋나 화면이 한쪽으로 쏠려 보인다.

**쓰는 쪽은 이름만 바꾼다** — `OverflowCell` → `CenteredOverflowCell`,
`SpanOverflowCell` → `CenteredSpanOverflowCell` (`panes/grid.tsx`). 왼쪽에 밀린 만큼을
오른쪽 여백으로 되돌려 놓는 일을 그 안에서 한다.

★ **폭은 재서 얻는다.** 왼쪽에 무엇이 있는지가 화면마다 다르다 — 아이콘 레일뿐인 곳도
  있고 하위 메뉴가 붙는 곳도 있으며, 하위 메뉴는 글자 길이와 언어를 따라 늘고 준다.
  그리드 안에서 이 칸이 얼마나 밀렸는지를 그대로 읽으므로 왼쪽에 무엇이 오든 맞는다.

★ **왼쪽 메뉴를 안 보이게 한 벌 더 그리는 길은 물렀다.** 폭은 정확했지만 그 빈 칸을
  그리드의 단으로 빼자 내용 칸이 창 끝까지 못 미쳐 **스크롤 막대가 화면 한가운데
  세로로 섰다.** Configure 는 하위 화면이 여덟이고 레일 메뉴가 상위(`configure.tsx`)에
  있어 베낄 수도 없다.

★ **폭을 꽉 채우는 화면에는 안 준다.** Keymap 의 키코드 목록은
  `repeat(auto-fill, 64px)` 로 창을 채우는 그리드다. 가운데 축이라 할 것이 없고,
  여백을 주면 열 수만 줄어 죽은 공간으로 보인다. 넣은 곳은 `ControlRow`(max-width 960)
  처럼 **폭이 정해진 가운데 열**을 쓰는 화면뿐이다.
